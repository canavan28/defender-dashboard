...
export function TechCapacity({ data }) {
  ...
  const techData = Object.entries(open.byTech)
    .filter(([id]) => resourceMap[parseInt(id)])
    .map(([id, count]) => ({ id, name: resourceMap[parseInt(id)], count }))
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        <MetricCard label="Active technicians" value={techData.length}
          delta="Assigned to open tickets" deltaDir="neutral" />
        ...
        <div className="space-y-4">
          {techData.map(tech => (
            <div key={tech.id} className="flex items-center gap-4">
              <span className="text-sm w-36 truncate flex-shrink-0"
                style={{ color: 'var(--text-secondary)' }}>
                {tech.name}
              </span>
              <div className="flex-1 rounded-full overflow-hidden"
                style={{ background: 'rgba(255,255,255,0.06)', height: 8 }}>
